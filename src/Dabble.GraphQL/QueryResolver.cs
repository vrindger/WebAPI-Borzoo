...
namespace Dabble.GraphQL
{
    ...
    public class QueryResolver : IQueryResolver
    {
        ...
        /// <inheritdoc />
        public async Task<StatisticDto> CreateStatisticAsync(ResolveFieldContext<object> context)
        {
            var dto = context.GetArgument<StatisticCreationDto>("statistic");

            var entity = (Statistic) dto;
            try
            {
                await _statisticRepo.AddAsync(entity, context.CancellationToken)
            ...
            return (StatisticDto)entity;
        }

        /// <inheritdoc />
        public async Task<bool> DeleteStatisticAsync(ResolveFieldContext<object> context)
        {
            var dto = context.GetArgument<StatisticCreationDto>("statistic");
            var entity = (Statistic)dto;
            Statistic statistic;
            try
            {
                statistic = await _statisticRepo.GetByNameAndYearAsync(entity.Country, entity.Year, context.CancellationToken)
                    .ConfigureAwait(false);
            }
            catch (EntityNotFoundException)
            {
                var err = new Error("Statistic not found.");
                context.Errors.Add(err);
                return false;
            }

            await _statisticRepo.DeleteStatisticAsync(statistic.Country, statistic.Year, context.CancellationToken)
                .ConfigureAwait(false);
        ...
        /// <inheritdoc />
        public async Task<StatisticDto> GetStatisticAsync(ResolveFieldContext<object> context)
        {
            string country = context.GetArgument<string>("country");
            string year = context.GetArgument<string>("year");
            Statistic entity;
            try
            {
                entity = await _statisticRepo.GetByNameAndYearAsync(country, year, cancellationToken: context.CancellationToken)
                    .ConfigureAwait(false);
            }
            catch (EntityNotFoundException)
            {
