...
namespace Dabble.Data.Mongo
{
    ...
    public static class MongoInitializer
    {
        ...
        {
            {
                ...
                await database.CreateCollectionAsync("Statistics", cancellationToken: cancellationToken)
                    .ConfigureAwait(false);
                var statisticsCollection = database.GetCollection<Statistic>("Statistics");
                var indexBuilder = Builders<Statistic>.IndexKeys;

                // create unique index "owner_list-name" on the fields "name" and "owner"
                var key = indexBuilder.Combine(
                    indexBuilder.Ascending(tl => tl.Country),
                    indexBuilder.Ascending(tl => tl.Year)
                );
                await statisticsCollection.Indexes.CreateOneAsync(
                    new CreateIndexModel<Statistic>(
                        key, new CreateIndexOptions { Name = "unique-country-year-idx", Unique = true }
        ...
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(Statistic)))
            {
                BsonClassMap.RegisterClassMap<Statistic>(map =>
                {
                    ...
                    map.MapProperty(u => u.Country).SetElementName("Country").SetOrder(1).SetIgnoreIfDefault(true);
                    map.MapProperty(u => u.Year).SetElementName("Year").SetIgnoreIfDefault(true);
                    map.MapProperty(u => u.Area).SetElementName("Area").SetIgnoreIfDefault(true);
                    map.MapProperty(u => u.Population).SetElementName("Population").SetIgnoreIfDefault(true);
                });
            }
